...
public:
	...
	UINT parseFromStr(LPCTSTR lpszString, 
					  bool &bIsOpeningTag, 
					  bool &bIsClosingTag,
					  bool &bIsInsideScript,
					  bool bParseAttrib = true);
	LPCTSTR GetTagStartPos(){ return m_pTagStartPos;};
	LPCTSTR GetTagEndPos(){ return m_pTagEndPos;};
...
private:
	...
	LPCTSTR				m_pTagStartPos;
	LPCTSTR				m_pTagEndPos;
	BOOL				m_bIsInline;//tells if it is like <tag /> ie <img /> <br />
};
									   ...
									   bool bParseAttrib /* = true */)
{
	...
	if(bIsInsideScript){
		if (!bClosingTag)
			return 0U;
		if(strTagName.CompareNoCase(_T("script")))
			return 0U;
	}

	//fix: rem white spaces till end </tagname   > or <tagname     >
	...
	// skip white-space after attribute/value pairs ie <tagname atrib=xx /  >
	while (::_istspace(*lpszEnd))
		lpszEnd = ::_tcsinc(lpszEnd);
