...
protected:
	...
	virtual bool parseTag(CLiteHTMLTag &rTag, bool &bIsOpeningTag, bool &bIsClosingTag,bool &bIsInsideScript);
...
inline bool CLiteHTMLReader::parseComment(CString &rComment)
{
	...
	// end of buffer?
	if (lpszEnd + (/*sizeof(TCHAR)*/1 * 2) >= m_lpszBuffer + m_dwBufLen)
		return (false);

	// skip white-space characters after comment ending delimeter '--'
	lpszEnd += (/*sizeof(TCHAR)*/1 * 2);
									  ...
									  bool &bIsInsideScript)
{
	...
	UINT nRetVal = rTag.parseFromStr(&m_lpszBuffer[m_dwBufPos], 
					bIsOpeningTag, bIsClosingTag, bIsInsideScript);
	if (!nRetVal)
		return (false);
